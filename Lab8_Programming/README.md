# ЛР8. Высокоуровневое программирование (C++, вариант 19)

Программа на C++ для процессора RISC-V (RV32I + Zicsr). Реализует индивидуальное
задание вариант 19 для пары периферии «переключатели + светодиоды»:
подсчитывает число непересекающихся пар `11` в двоичном представлении значения
с переключателей и выводит результат на светодиоды.

## Мои файлы (решение)
- `main.cpp` — программа: `main` (пустой бесконечный цикл) и
  `extern "C" int_handler` (по прерыванию читает переключатели `sw_ptr->value`,
  считает пары `11`, пишет результат в `led_ptr->value`).
...
count = 0
while value != 0:
    if (value & 0b11) == 0b11:   # нашли пару "11"
        count++; value >>= 2     # сдвиг на 2 — пары не пересекаются
    else:
        value >>= 1
```
...
3. Симулировать тестбенчем `lab_13_tb_processor_system`: при изменении `sw_i`
   приходит прерывание, процессор читает переключатели и выводит число пар `11`
   на `led_o`.
