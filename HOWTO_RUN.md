...
Файл `HW_RV32I_asm/hw_variant19.txt` — это НЕ Vivado, запускается в эмуляторе:
...
Вариант 19: подсчёт непересекающихся пар `11`. Вход/результат в ОЗУ
(`0x10010000` / `0x10010004`), результат продублирован в `x10`.
Пример: `input = 0xDEAD` → результат `4`.
